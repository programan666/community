...
function open_industry_list_page() {
	loadTabMenu('html/m-industry.html', loadIndustryList)
}

function loadIndustryList() {
	$(function() {
		var table = $("#industrydatatable").DataTable({
			"processing": true,
			"serverSide": true,
			'ajax': {
				'contentType': 'application/json',
				'url': callurl + "/industry/tablelist",
				'type': 'POST',
				'data': function(d) {
			...
			"columns": [{
					...
					"render": function(data, type, full, meta) { //render改变该列样式,4个参数，其中参数数量是可变的。
						return '<button name="industryDetail" class="btn btn-danger btn-sm btn-row" data-id=' + data + '>编 辑</button>';
					}
				}
				//data指该行获取到的该列数据
		...
		function initComplete(data) {
			...
			$('button[name="industryDetail"]').on('click', function() {
				var id = $(this).data("id");
				open_industry_detail_page(id);
			});
		}
	});

	$('#purgeIndustryBtn').click(function() {
	...
	$('#addIndustryBtn').click(function(){
		open_industry_detail_page();
	});

}

function deleteIndustry(delid) {
	$.ajax({
		type: "post",
		url: callurl + "/industry/delete/" + delid,
		async: true,
		dataType: 'json',
...
function open_industry_detail_page(id) {
    loadTabMenu('html/m-industry-detail.html', function() {
         ...
         * 保存分类
         */
        $("#saveIndustryBtnSubmit").on('click', function(){
            saveIndustry();
        });
         ...
         * 取消保存
         */
        $("#cancelIndustryBtnSubmit").on('click', function () {
            open_industry_list_page();
        });
    })
}
...
function getIndustryDetail(id) {
	$.ajax({
        type: "get",
        url: callurl + "/industry/detail/" + id,
        async: true,
        dataType: 'json',
        contentType: 'application/json; charset=UTF-8',
        success: function(data) {
