...
//参数说明：showInputId:返回的url显示的控件id, filePath：上传路径,  inputFile: 输入文件id
function mArticleUploadFile(showInputId, filePath, inputFile) {
	var formData = new FormData();
	formData.append("file", $("#" + inputFile)[0].files[0]);
	formData.append("path", filePath);
	$.ajax({
		...
		success: function(data) {
			...
			$('#editor .w-e-panel-tab-content .w-e-button-container .right').click();
			$('#' + modelId).modal('hide');
		},
		error: function() {
			console.log("上传出现异常");
